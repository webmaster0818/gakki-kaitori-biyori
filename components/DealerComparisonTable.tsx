...
export default function DealerComparisonTable({
  dealerIds,
  heading = "楽器買取業者 主要7社 徹底比較",
  intro = "店舗数・対応エリア・査定スピード・手数料の公式公開情報をまとめました。出典は各社公式サイト（最終確認：2026年6月）。",
}: Props) {
  ...
  return (
    <section className="my-10 not-prose">
      ...
      {/* デスクトップ：横スクロールテーブル */}
      <div className="hidden md:block overflow-x-auto border border-warm-border rounded-xl bg-white">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-accent text-cream">
              ...
              <th className="px-4 py-3 text-left font-bold whitespace-nowrap">楽器の専門性</th>
          ...
          <tbody>
            {dealers.map((d) => (
              <tr key={d.id} className="border-t border-warm-border hover:bg-cream/60">
                ...
                <td className="px-4 py-3">
                  <a
                    ...
                    className="inline-block bg-gold text-foreground text-xs font-bold px-3 py-2 rounded-lg hover:bg-gold-dark hover:text-white transition-colors whitespace-nowrap"
                  >
                    公式サイト
                  </a>
                </td>
          ...
          >
            <div className="flex items-start justify-between gap-3 mb-3">
              ...
              <a
                ...
                target="_blank"
                rel="noopener noreferrer sponsored"
                className="inline-block bg-gold text-foreground text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-gold-dark hover:text-white transition-colors whitespace-nowrap"
              >
                公式
              </a>
            </div>
            <dl className="grid grid-cols-[6.5rem_1fr] gap-y-2 gap-x-3 text-xs leading-relaxed">
              ...
              <dt className="text-accent-dark font-bold">楽器の専門性</dt>
      ...
      <p className="text-[11px] text-warm-gray mt-3 leading-relaxed">
        ※ 上記情報は各業者の公式サイトに記載の内容を 2026年6月時点で確認したものです。最新情報は必ず公式サイト（
        {dealers.map((d, i) => (
          <span key={d.id}>
            <a
              href={d.source}
